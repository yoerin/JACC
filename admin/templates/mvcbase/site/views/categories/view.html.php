...
class ##Component##ViewCategories extends JViewLegacy
{
	...
	protected function _prepareDocument()
	{
		...
		if (empty($title)) {
			$title = htmlspecialchars_decode($config->get('sitename'));
		} elseif ($config->get('sitename_pagetitles', 0)) {
			$title = JText::sprintf('JPAGETITLE', htmlspecialchars_decode($config->get('sitename')), $title);
		}
